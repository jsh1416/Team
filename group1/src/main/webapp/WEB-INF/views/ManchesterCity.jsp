...
<style>
	.firstSection::before{
		content: "";
        background: url(${cpath}/resources/images/background/ManchesterCity/4.jpg);
	...
	<div id="fullpage">
		<div class="section firstSection" id="MC1">
			<div style="position: absolute; top: 100px;">
				<h1 style="display: inline-block; margin-right: 10px">Manchester City</h1>
...
<script>
	...
	document.getElementById('correctNews').onclick = function(event) {
		...
		const url = '${cpath}/api/mc/correct' 			// 여기서만 team, type 매개변수 바꿔주면 됨
		const opt = {
			method: 'GET',	
		}
		fetch(url, opt)				
	...
	document.getElementById('latestNews').onclick = function(event) {
		document.querySelector('div.news').innerText='' // 내용 비우기
		const url = '${cpath}/api/mc/latest' 			// 여기서만 team, type 매개변수 바꿔주면 됨
			const opt = {
				method: 'GET',	
			}
			fetch(url, opt)				
