...
<style>
	.firstSection::before{
		content: "";
        background: url(${cpath}/resources/images/background/Liverpool/4.jpg);
		...
		content: "";
        background: url(${cpath}/resources/images/background/Liverpool/2.jpg);
        background-size: cover;
        opacity: 0.5;
        position: absolute;
        top: 0px;
		...
		.thirdSection::before{
		content: "";
        background: url(${cpath}/resources/images/background/Liverpool/3.jpg);
        background-size: cover;
        opacity: 0.5;
        position: absolute;
        top: 0px;
	...
	<div id="fullpage">
		<div class="section firstSection" id="LIV1">
			<div style="position: absolute; top: 100px;">
				<h1 style="display: inline-block; margin-right: 10px">Liverpool News</h1>
		...
		<div class="section secondSection" id="LIV2">
			<h2>liv2</h2>
			...
			<div class="wrap-table100">
				<div class="table100 ver1 m-b-110"">
						...
						<table>
							<thead>
								<tr class="row100 head" >
									<th class="cell100 column1" style="background:#E31B23;"><button style="color: #FFFFFF;" id="newNumber" >최신</button></th>
									<th class="cell100 column2" style="background:#E31B23;"><button style="color: #FFFFFF;" id="viewNumber" >조회수</button></th>
									<th class="cell100 column3" style="background:#E31B23;"><button style="color: #FFFFFF;" id="likeNumber">추천수</button></th>
								</tr>
							</thead>
						</table>
					</div>
					...
					<div class= "sb" style="background:#E31B23;">
							<form >
								<span >
								<select  name="type" style="background:#E31B23;">
								...
								<span><input style="background:#E31B23;" type="text" name="search" value="${param.search }" ></span>
                        		<span><input style="background:#E31B23;" type="submit" value="검색"></span>
                    			 </form>
...
<script>
	...
	document.getElementById('correctNews').onclick = function(event) {
		...
		const url = '${cpath}/api/liv/correct' 			// 여기서만 team, type 매개변수 바꿔주면 됨
		const opt = {
			method: 'GET',	
		}
		fetch(url, opt)				
			...
			const newslist = JSON.parse(json) // string -> obj
   			if(newslist == null) {
				...
				for(let i = 0; i < newslist.items.length; i++){
					...
					const newstitle = '<a href=' + newslist.items[i].link + ' target="_blank" style="color:#E31B23; font-weight:bold; cursor: pointer;">' 
	...
	document.getElementById('latestNews').onclick = function(event) {
		document.querySelector('div.news').innerText='' // 내용 비우기
		const url = '${cpath}/api/liv/latest' 			// 여기서만 team, type 매개변수 바꿔주면 됨
			const opt = {
				method: 'GET',	
			}
			fetch(url, opt)				
