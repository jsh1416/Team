...
<style>
	.firstSection::before{
		content: "";
        background: url(${cpath}/resources/images/background/Chelsea/4.png);
...
$.ajax({
	  ...
	  dataType: 'json',
		...
		for(let i = 0; i < matchday.matches.length; i++) { // 팀 고유 id를 받아와서 비교
			if(matchday.matches[i].homeTeam.id == 61 || matchday.matches[i].awayTeam.id == 61){
				matchday.matches[i].homeTeam.id == 61 ? isHome=true : isHome=false // 해당팀이 홈팀인지 어웨이팀인지 구분
