...
<style>
	.firstSection::before{
		...
		content: "";
        background: url(${cpath}/resources/images/background/Tottenham/2.jpg);
        background-size: cover;
        opacity: 0.5;
        position: absolute;
        top: 0px;
		...
		.thirdSection::before{
		content: "";
        background: url(${cpath}/resources/images/background/Tottenham/3.jpg);
        background-size: cover;
        opacity: 0.5;
        position: absolute;
        top: 0px;
...
// 						seriesType: 'bars',
	...
	<div id="fullpage">
		<div class="section firstSection" id="tto1">
			<div class="mainflex">
				<div id="s_left">
	...
	<div class="section secondSection" id="tto2">
						...
						<table>
							<thead>
								<tr class="row100 head" >
									...
									<th class="cell100 column2" style="background:${clubColor};"><a href="${cpath }/5?param=viewOrder#thirdPage" style="color: #FFFFFF;"><h6>View</h6></a></th>
									<th class="cell100 column3" style="background:${clubColor};"><a href="${cpath }/5?param=likeOrder#thirdPage" style="color: #FFFFFF;"><h6>Like</h6></a></th>
									<th class="cell100 column3" style="background:${clubColor};"><button style="color: #FFFFFF;" id="writer"><h6>Writer</h6></button></th>
									<th class="cell100 column3" style="background:${clubColor};"><a href="${cpath }/5?param=newOrder#thirdPage" style="color: #FFFFFF;"><h6>Date</h6></a></th>		
								</tr>
							</thead>
						</table>
					</div>
...
$.ajax({
	  ...
	  dataType: 'json',
		...
		for(let i = 0; i < matchday.matches.length; i++) { // 팀 고유 id를 받아와서 비교
			if(matchday.matches[i].homeTeam.id == 73 || matchday.matches[i].awayTeam.id == 73){
				matchday.matches[i].homeTeam.id == 73 ? isHome=true : isHome=false // 해당팀이 홈팀인지 어웨이팀인지 구분
...
<script>
//해당팀 관련 뉴스 띄우기 jsh
const url = '${cpath}/api/tto/correct' 			// 여기서만 team, type 매개변수 바꿔주면 됨
		const opt = {
			method: 'GET',	
		}
		fetch(url, opt)				
...
document.getElementById('latestNews').onclick = function(event) {
	const url = '${cpath}/api/tto/latest' 			// 여기서만 team, type 매개변수 바꿔주면 됨
	const opt = {
		method: 'GET',	
	}
	fetch(url, opt)				
